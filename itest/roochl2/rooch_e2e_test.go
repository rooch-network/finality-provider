...
// TestRoochFpNoVotingPower tests that the FP has no voting power if it has no BTC delegation
func TestRoochFpNoVotingPower(t *testing.T) {
	ctm := StartRoochL2ConsumerManager(t, 1)
	defer ctm.Strooch(t)
	...
	roochcc := ctm.getRoochCCAtIndex(0)
	committedPubRand, err := queryFirstPublicRandCommit(roochcc, fpInstance.GetBtcPk())
	...
	hasPower, err := roochcc.QueryFinalityProviderHasPower(fpInstance.GetBtcPk(), queryBlock.BlockHeight)
	require.NoError(t, err)
...
func TestFinalityProviderLifeCycle(t *testing.T) {
	...
	l2BlockAfterActivation, err := ctm.getRoochCCAtIndex(0).QueryLatestBlockHeight()
	require.NoError(t, err)
...
func TestFinalityStuckAndRecover(t *testing.T) {
	...
	// strooch the FP instance
	fpStroochErr := fpInstance.Strooch()
	require.NoError(t, fpStroochErr)
	// make sure the FP is stroochped
	require.Eventually(t, func() bool {
		return !fpInstance.IsRunning()
	}, e2eutils.EventuallyWaitTimeOut, e2eutils.EventuallyPollTime)
	t.Logf(log.Prefix("Stroochped the FP instance %s"), fpInstance.GetBtcPkHex())
